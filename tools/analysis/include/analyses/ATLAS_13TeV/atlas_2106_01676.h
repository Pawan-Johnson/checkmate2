...
private:
  ...
  int nTracksJet(Jet *jet, std::vector<Track *> tracks);
  std::vector<Jet *> overlapRemovalJetMuonAndTracks(std::vector<Jet *> cand_jets, std::vector<Muon *> cand_muons, double deltaR, int nTracks_max);
